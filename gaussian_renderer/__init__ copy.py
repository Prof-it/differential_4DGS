...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, frame, raw_render = False, scaling_modifier = 1.0, separate_sh = False, override_color = None, use_trained_exp=False):
    ...
    screenspace_points = torch.zeros_like(pc.get_xyz(frame, raw_render), dtype=pc.get_xyz(frame, raw_render).dtype, requires_grad=True, device="cuda") + 0
    try:
    ...
    means3D = pc.get_xyz(frame, raw_render)
    ...
    if override_color is None:
        if pipe.convert_SHs_python:
            ...
            dir_pp = (pc.get_xyz(frame, raw_render) - viewpoint_camera.camera_center.repeat(pc.get_features(frame, raw_render).shape[0], 1))
