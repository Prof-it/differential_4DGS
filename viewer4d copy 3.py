...
class viewer4d:
                ...
                try:
                    ...
                    if play_pause:
                        ...
                        if interpolation_index >= 1:
                            ...
                            self.fill_buffer(future_frame, self.frames[next_frame], False)
                            start_time = time.perf_counter()
    ...
    def fill_frame_to_gaussian_obj(self, viewframe, gaussian_obj, gaussian_frame):
    ...
    def load_data_to_gaussians(self):
        #find how many frames there are
        i = 0
        temp_gaussians = GaussianModel(100,2,0)
        temp_gaussians.current_reference_counts = [None]*100
        while(True):
            print("loading frame " + str(i))
            frame_path = os.path.join(self.source_path, "frame" + str(i) + ".ply")
            frame_references_path = os.path.join(self.source_path, "frame" + str(i) + "_references.move")
            ...
            self.gaussians.active_sh_degree = import_pkg[6]
            tmp_frame = gaussian_frame.GaussianFrame(new_xyz, new_features_dc, new_features_rest, new_scaling, new_rotation, new_opacity, new_reference_counts=new_reference_counts)
            if(i==0):
                self.fill_frame_to_gaussian_obj(0, temp_gaussians, tmp_frame)
            else:
                self.fill_frame_to_gaussian_obj(i, temp_gaussians, tmp_frame)

            
            self.frames.append(gaussian_frame.GaussianFrame(
                temp_gaussians.get_xyz(i), 
                temp_gaussians.get_features_dc(i), 
                temp_gaussians.get_features_rest(i), 
                temp_gaussians.get_scaling(i), 
                temp_gaussians.get_rotation_unnormalized(i), 
                temp_gaussians.get_opacity(i), new_reference_counts=new_reference_counts))
