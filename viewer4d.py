...
class viewer4d:
    ...
    def start_viewer(self, args, pipe):
        ...
        for i in range(0,self.buffer_size):
            self.fill_buffer(i, self.frames[i])
                ...
                try:
                    ...
                    if(play_pause):
                        if(not do_scale_points):
                            interpolation_index = scaling_modifer
            
                        
                        #print("pause")
                        #if(not do_scale_points):
                        #    real_frame = round(scaling_modifer * args.total_frames) - 1
                        #if(real_frame < 0):
                        #    real_frame = 0
                        #new_current_batch = 0 if real_frame == 0 else math.floor(real_frame / self.buffer_size)
                        #view_frame = real_frame - new_current_batch * self.buffer_size
                        #if(new_current_batch != current_batch):
                            #adjust main base frame
                        #    if(new_current_batch == 0):
                        #        self.fill_buffer(0, self.frames[0])
                         #   else:
                         #       previous_frame = (self.buffer_size - 1) * new_current_batch
                        #        self.fill_buffer(0, gaussian_frame.GaussianFrame(self.gaussians.get_xyz(previous_frame).clone(), self.gaussians.get_features_dc(previous_frame).clone(), self.gaussians.get_features_rest(previous_frame).clone(), self.gaussians.scaling_inverse_activation(self.gaussians.get_scaling(previous_frame)).clone(), self.gaussians.get_rotation_unnormalized(previous_frame).clone(), self.gaussians.inverse_opacity_activation(self.gaussians.get_opacity(previous_frame)).clone(), new_reference_counts=self.frames[0].reference_counts.clone()))

                    else:
                        interpolation_index = (time.time() - start_time) / ((1/args.frame_rate) * args.skip_frame)
                        if(interpolation_index >= 1):
                            ...
                            #prepare frame to interpolate to
                            future_frame = view_frame + 1 * args.skip_frame #however many frames should be skipped
                            ...
                            if(future_frame >= self.buffer_size or real__future_frame >= self.total_frames):
                                if(real__future_frame>=self.total_frames):
                                    #future frame is end of sequence. Can't interpolate to first frame again so need to start over somehow
                                    view_frame = 0
                                    future_frame = 1 + 1 * args.skip_frame
                                    additional_batch = 0 
                                    if(future_frame >= args.buffer_size):
                                        additional_batch = math.floor(future_frame / args.buffer_size)
                                        future_frame = future_frame - additional_batch * args.buffer_size
                                    current_batch = 0 + additional_batch
                                    self.fill_buffer(0, self.frames[0]) 
                                else:
                                    #view_frame = future_frame                                   
                                    additional_batch = math.floor(future_frame / args.buffer_size)
                                    future_frame = 1+ future_frame - additional_batch * args.buffer_size

                                    full_base_frame = current_batch * args.frames_per_batch -1
                                    base_frame = len(self.gaussians._xyz)- 1 # view_frame-1#real__future_frame - future_frame
                                    if args.skip_frame > 1:
                                        base_frame = old_view_frame
                                    #base_frame = view_frame -1

                                    current_batch = math.floor(real__future_frame / self.buffer_size) -1 + additional_batch     
                                         
                                    self.fill_buffer(0, gaussian_frame.GaussianFrame(self.gaussians.get_xyz(base_frame, (not keep_alive)).clone(), self.gaussians.get_features_dc(base_frame, (not keep_alive)).clone(), self.gaussians.get_features_rest(base_frame, (not keep_alive)).clone(), self.gaussians.scaling_inverse_activation(self.gaussians.get_scaling(base_frame, (not keep_alive))).clone(), self.gaussians.get_rotation_unnormalized(base_frame,(not keep_alive)).clone(), self.gaussians.inverse_opacity_activation(self.gaussians.get_opacity(base_frame, (not keep_alive))).clone(), new_reference_counts=self.frames[0].reference_counts.clone()))
                                    #self.fill_buffer(view_frame, self.frames[real__future_frame])
                    ...
                    if(do_training): #train button for framerate change
                        args.frame_rate = (scaling_modifer * 60) + ((1-scaling_modifer) * 0.1) 
                    else:
                        args.skip_frame = math.floor((scaling_modifer * 1) + ((1-scaling_modifer) * 9))
                    print("skipping frames")
                    print(args.skip_frame)
                    print("frame_rate")
                    print(args.frame_rate)
                    if(not do_scale_points):
                        scaling_modifer = 1
                    if custom_cam != None and 1==1:
                        if(not args.do_interpolate):
                            interpolation_index = 0 
                        net_image = render_interpolated(custom_cam, self.gaussians, pipe, background, view_frame + interpolation_index, future_frame, raw_render=(not keep_alive), scaling_modifier=scaling_modifer, use_trained_exp=False, separate_sh=SPARSE_ADAM_AVAILABLE)["render"]# convert_tensor(difference_matte_helper.visualize_points_projection(gaussians.get_xyz(frame), viewpoint_cam)) #render(custom_cam, gaussians, pipe, background, view_frame, scaling_modifier=scaling_modifer, use_trained_exp=dataset.train_test_exp, separate_sh=SPARSE_ADAM_AVAILABLE)["render"]
                        net_image_bytes = memoryview((torch.clamp(net_image, min=0, max=1.0) * 255).byte().permute(1, 2, 0).contiguous().cpu().numpy())

                    network_gui.send(net_image_bytes, os.path.join(args.source_path, "..", "..", "..", "point", "colmap_0"))#os.path.join(self.source_path, args.frame_folder_name+str((args.start_frame))))
                except Exception as e:
                    print(e)
                    network_gui.conn = None


    def fill_buffer(self, viewframe, gaussian_frame):
